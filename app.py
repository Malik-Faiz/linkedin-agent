...
def _li_account_picker_page(username, slot, access_token, expires_in, personal_name, personal_id, orgs, oauth_url=''):
    ...
    for org in orgs:
        oid   = org.get("id", "")
        oname = org.get("name", f"Company {oid}").replace("'", "\\'").replace('"', '&quot;')
        org_cards += f"""<label class="acct-card" onclick="pick('organization','{oid}','{oname}')"><span class="acct-ico">🏢</span><span class="acct-info"><strong>{org.get("name",f"Company {oid}")}</strong><small>Company Page · ID: {oid}</small></span><span class="acct-arrow">→</span></label>"""
    ...
    org_section = f'<div class="divider"><span>OR COMPANY PAGE ({len(orgs)} found)</span></div><div class="org-scroll">{org_cards}</div>' if orgs else '<div class="no-orgs"><span style="font-size:20px;">🏢</span><span>No company pages found on this account</span></div>'
...
<div class="manual-box"><div class="manual-hdr">🏢 Connect a Company Page manually</div><div class="manual-sub">Paste your Company Page numeric ID below — find it in your page URL:<br><code>linkedin.com/company/<strong>12345678</strong>/admin</code></div><div class="manual-row"><input type="text" id="manualPageId" placeholder="e.g. 12345678" maxlength="30" oninput="this.value=this.value.replace(/[^0-9]/,'')"><input type="text" id="manualPageName" placeholder="Page display name (e.g. My Company)"></div><div id="manualErr" style="display:none;color:#ff8080;font-size:11px;margin-top:6px;"></div><button class="manual-btn" onclick="pickManual()">🔗 Connect Company Page →</button></div>
...
async function pickManual(){{const pageId=document.getElementById('manualPageId').value.trim();const pageName=document.getElementById('manualPageName').value.trim();const errEl=document.getElementById('manualErr');errEl.style.display='none';if(!pageId){{errEl.textContent='⚠ Please enter a Company Page ID.';errEl.style.display='block';return;}}if(!/^[0-9]+$/.test(pageId)){{errEl.textContent='⚠ Page ID must be numbers only.';errEl.style.display='block';return;}}const name=pageName||('Company Page '+pageId);document.getElementById('loadingBox').style.display='block';document.querySelector('.manual-btn').disabled=true;try{{const res=await fetch('/api/auth/linkedin/pick',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{username:'{username}',slot:{slot},acct_type:'organization',acct_id:pageId,acct_name:name}})}});const data=await res.json();if(data.ok){{window.opener&&window.opener.postMessage('channel_connected:linkedin:{slot}','*');window.close();}}else{{errEl.textContent='Error: '+(data.message||'Unknown error');errEl.style.display='block';document.getElementById('loadingBox').style.display='none';document.querySelector('.manual-btn').disabled=false;}}}}catch(e){{errEl.textContent='Network error: '+e.message;errEl.style.display='block';document.getElementById('loadingBox').style.display='none';document.querySelector('.manual-btn').disabled=false;}}}}
