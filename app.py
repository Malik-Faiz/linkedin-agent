...
def send_to_buffer(username, post_text, image_url=None):
    ...
    for slot, cid, name in channels:
        ...
        if status != 200:
            add_log(username, f"  → [{name}] HTTP {status} error: {result}", "error"); continue
        if "errors" in result:
        ...
        add_log(username, f"  → [{name}] Queued ✓" + (f" ID: {pid}" if pid else " (no ID)"), "ok")
        success += 1
    return success
...
def run_batch(username, triggered_by="scheduler"):
    state = get_state(username)
    if state["running"]:
        add_log(username, "Batch already running.", "warn"); return
    state["running"] = True; state["status"] = "running"; state["today_count"] = 0
    ...
    for j, subject in enumerate(batch):
        manual_image_url = None; base_subject = subject
        if "| IMG:" in subject:
            parts = subject.split("| IMG:")
            base_subject = parts[0].strip(); manual_image_url = parts[1].strip()
        ...
        if manual_image_url:
            ...
            if image_url: add_log(username, f"[{j+1}/{len(batch)}] Image fetched via SerpAPI ✓", "ok")
        sent = send_to_buffer(username, post_text, image_url)
        if sent > 0:
            add_log(username, f"[{j+1}/{len(batch)}] Sent to {sent} channel(s) ✓", "ok")
            state["today_count"] += 1; state["total_run"] += sent
        else:
...
def keep_alive_loop():
    ...
    while True:
        try:
            ...
            url = f"https://{railway_url}" if railway_url else (render_url or f"http://localhost:{port}")
...
def register():
    ...
    if utc_offset is not None:
        cfg = load_config(username); cfg["utc_offset_hours"] = float(utc_offset); save_config(username, cfg)
...
def get_config(username):
    ...
    for k, v in cfg.items():
        if k in no_mask: masked[k] = v
        elif isinstance(v, str) and v and len(v) > 8: masked[k] = "*" * (len(v) - 4) + v[-4:]
        else: masked[k] = v
...
def save_config_route(username):
    data = request.get_json(); cfg = load_config(username)
    for field in ["groq_api_key", "buffer_api_key", "serpapi_key"]:
        val = (data.get(field) or "").strip()
        if val and not val.startswith("*") and "*" not in val: cfg[field] = val
    for i in [1, 2, 3, 4]:
        cid  = (data.get(f"buffer_channel_{i}") or "").strip()
        name = (data.get(f"buffer_channel_{i}_name") or "").strip()
        if cid and "*" not in cid: cfg[f"buffer_channel_{i}"] = cid
        if name and "*" not in name: cfg[f"buffer_channel_{i}_name"] = name
    if "active_channels" not in cfg: cfg["active_channels"] = [1]
...
def toggle_channel(username):
    data = request.get_json(); ch = data.get("channel")
    ...
    if not isinstance(active, list): active = [active]
    if ch in active:
        if len(active) <= 1: return jsonify({"ok": False, "message": "At least one channel must be active"}), 400
        active.remove(ch); state = "OFF"
    else:
        active.append(ch); active.sort(); state = "ON"
    cfg["active_channels"] = active; save_config(username, cfg)
...
def delete_channel(username):
    ...
    cfg = load_config(username)
    name = cfg.get(f"buffer_channel_{ch}_name", f"Channel {ch}")
    for key in [f"buffer_channel_{ch}", f"buffer_channel_{ch}_name"]: cfg.pop(key, None)
    active = cfg.get("active_channels", [1])
    if not isinstance(active, list): active = [active]
...
def get_status(username):
    ...
    if not isinstance(active_chs, list): active_chs = [active_chs]
    channels_info = []
    for i in [1, 2, 3, 4]:
        cid  = cfg.get(f"buffer_channel_{i}", "").strip()
        name = cfg.get(f"buffer_channel_{i}_name", f"Channel {i}")
        channels_info.append({"slot": i, "id": cid, "name": name,
                               "active": (i in active_chs) and bool(cid), "exists": bool(cid)})
    return jsonify({
        "status": state["status"], "running": state["running"],
        "today_count": state["today_count"], "total_run": state["total_run"],
        "last_run": state["last_run"], "next_run_iso": next_run.isoformat() + "Z",
        "seconds_left": max(0, int((next_run - datetime.utcnow()).total_seconds())),
        "subjects": subjects, "logs": state["logs"][-30:],
...
def manual_run(username):
    state = get_state(username)
    if state["running"]: return jsonify({"ok": False, "message": "Already running"}), 409
    ...
    if mode == "manual_image" and image_b64:
        try:
            ...
            res = requests.post("https://freeimage.host/api/1/upload", data={
                "key": "6d207e02198a847aa98d0a2a901485a5", "action": "upload",
                "source": image_b64, "format": "json"
            }, timeout=30)
            if res.status_code == 200:
                rj = res.json()
                if "image" in rj: uploaded_url = rj["image"]["url"]; add_log(username, f"Upload success! URL: {uploaded_url}", "ok")
                else: add_log(username, f"Upload failed: {res.text}", "error")
            else: add_log(username, f"Upload HTTP {res.status_code}: {res.text}", "error")
        except Exception as e: add_log(username, f"Image upload error: {e}", "error")
    subjects_file = user_subjects_path(username)
    with open(subjects_file, "a", encoding="utf-8") as f:
        for s in new_subjects:
            line = s.strip()
            if mode == "auto_image": line = f"{line} (create image)"
            elif mode == "manual_image" and uploaded_url: line = f"{line} | IMG: {uploaded_url}"
            f.write(line + "\n")
...
def delete_subject(username):
    data  = request.get_json(); index = data.get("index")
    subjects_file = user_subjects_path(username)
    if not os.path.exists(subjects_file): return jsonify({"ok": False, "message": "No subjects"}), 404
    with open(subjects_file, "r", encoding="utf-8") as f:
        subjects = [l.strip() for l in f if l.strip()]
...
if __name__ == "__main__":
    ...
    print(f"[STARTUP] Routes: / → /login → /register → /api-setup → /dashboard")
